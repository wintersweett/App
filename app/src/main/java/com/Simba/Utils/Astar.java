...
public class Astar {
    ...
    //开放列表 白格子
    private Queue<Node>openList=new PriorityQueue<>();//priorityQueue是排好序的队列
    //关闭列表，黄格子
    List<Node> closeList=new ArrayList<>();
    /*
    计算h值
     */
    private int calH(Coord end,Coord coord) {
    ...
    //从open中查找
    private Node findInOpen(Coord coord) {
    ...
    //判断是否在close中
    private  boolean isInClose(Coord coord) {
        return coord !=null && isInClose(coord.x,coord.y);
    }
...
//开始搜索
    ...
    private void moveNodes(MapInfo mapInfo) {
        while (!openList.isEmpty()) {
            //如果已经是终点了
            if (isInClose(mapInfo.end.coord)) {
                //统计结果
                calPath(mapInfo.map,mapInfo.end);
            }
            //把open中最小的一个取出来，放入close中
    ...
    private void addNeighborNodeInOpen(MapInfo mapInfo, Node current, int x, int y, int directValue) {
        if (canAddNodeToOpen(mapInfo,x,y)) {//是否可以通过
            ...
            Node child=findInOpen(coord);
            if (child==null) {//如果能填入数字
                int h=calH(end.coord,coord);
    ...
    private void calPath(int[][] map, Node end) {
